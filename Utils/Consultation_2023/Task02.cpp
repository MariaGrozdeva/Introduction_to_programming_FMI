...
void initMatrix(char**& matrix, size_t rowsCount)
{
...
void freeMatrix(char** matrix, size_t rowsCount)
{
	for (size_t i = 0; i < rowsCount; i++)
	{
		delete[] matrix[i];
...
void printPermutationOfWords(const char* const* words, size_t rowsCount)
{
	for (size_t i = 0; i < rowsCount; i++)
	{
		std::cout << words[i] << ' ';
...
void generateAllPermutationsOfWords(char** words, size_t rowsCount, unsigned int pos)
{
	if (pos == rowsCount)
	{
...
void printAllPermutationsOfWords(char** words, size_t rowsCount)
{
